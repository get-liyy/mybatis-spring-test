            ...
            <table class="gridtable" style="width:100%;text-align: center;">
                <tr>
                    <c:if test="${pageInfo.hasPreviousPage}">
                        <td>
                            <a href="${pageContext.request.contextPath}/index?page=${pageInfo.prePage}&rows=${pageInfo.pageSize}&countryName=${queryParam.countryName}&countryCode=${queryParam.countryCode}">前一页</a>
                        </td>
                    </c:if>
                    <c:forEach items="${pageInfo.navigatepageNums}" var="nav">
                        ...
                        <c:if test="${nav != pageInfo.pageNum}">
                            <td>
                                <a href="${pageContext.request.contextPath}/index?page=${nav}&rows=${pageInfo.pageSize}&countryName=${queryParam.countryName}&countryCode=${queryParam.countryCode}">${nav}</a>
                            </td>
                        </c:if>
                    </c:forEach>
                    <c:if test="${pageInfo.hasNextPage}">
                        <td>
                            <a href="${pageContext.request.contextPath}/index?page=${pageInfo.nextPage}&rows=${pageInfo.pageSize}&countryName=${queryParam.countryName}&countryCode=${queryParam.countryCode}">下一页</a>
                        </td>
                    </c:if>
                </tr>
            </table>
